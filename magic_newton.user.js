...
async function autoBuy() {
  ...
  var up = Array.from(document.querySelectorAll("div p")).filter(el => el.textContent.includes("Roll now"));
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(6000, 7100));
  }

  up = Array.from(document.querySelectorAll("div p")).filter(el => el.textContent.includes("Let's roll"));
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(6000, 7100));
  }

  up = Array.from(document.querySelectorAll("div p")).filter(el => el.textContent.includes("Throw Dice"));
  if (up.length != 0){
    triggerEvents(up[0]);
  ...
  for(var i = 0; i < 6; i++) {
    up = Array.from(document.querySelectorAll("div p")).filter(el => el.textContent.includes("Press"));
    if (up.length != 0){
      triggerEvents(up[0]);
      await sleep(getRandomDelay(9000, 10100));
    }
