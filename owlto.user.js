...
// @version      0.1
// @author       IvanAgafonov
// @match        https://owlto.finance/
// @downloadURL  https://github.com/IvanAgafonov/test-violentmonkey/raw/main/owlto.user.js
// @updateURL    https://github.com/IvanAgafonov/test-violentmonkey/raw/main/owlto.user.js
...
async function autoBuy() {
  ...
  var up = Array.from(document.querySelectorAll("div")).filter(el => el.textContent == " Connect Wallet " && el.className == "connect");
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(3000, 4000));
  }

  up = Array.from(document.querySelectorAll("div span")).filter(el => el.textContent == "Rabby Wallet");
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(16000, 17000));
  }

  up = Array.from(document.querySelectorAll("img[src='/assets/checkIn-b1e716a6.svg']"));
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(4000, 5000));
  }

  up = Array.from(document.querySelectorAll("div.select-div-select-left"));
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(4000, 5000));
  }
  ...
  up = Array.from(document.querySelectorAll("img[src='/assets/checkin-b9854232.png']"));
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(4000, 5000));
  }
