...
async function autoBuy() {
  ...
  for(var i=0;i<15;i++) {
    ...
    var up = Array.from(document.querySelectorAll("span.whitespace-nowrap.flex.items-center"));
    for (var item of up) {
      if (parseInt(item.textContent) <= 30 && !item.textContent.includes('k') && !item.textContent.includes('K') && !item.textContent.includes('M')  && !item.textContent.includes('m')) {
        triggerEvents(item);
        await sleep(getRandomDelay(3000, 4100));
        var up2 = Array.from(document.querySelectorAll("button")).filter(el => el.textContent == "Purchase");
        if (up2.length != 0){
          triggerEvents(up2[0]);
          await sleep(getRandomDelay(4200, 4500));
        }
      }
    }
...
function initializeScript() {
    ...
    setTimeout(autoBuy, getRandomDelay(3000, 4050));
}
