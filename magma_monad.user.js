...
// @updateURL    https://github.com/IvanAgafonov/test-violentmonkey/raw/main/magma_monad.user.js
...
async function autoBuy() {
  ...
  up = Array.from(document.querySelectorAll("button div div div")).filter(el => el.textContent == "Rabby Wallet");
  if (up.length != 0){
    triggerEvents(up[0]);
  ...
  var mon = 0;
  if (up.length != 0){
    mon = parseInt(up[0].textContent)
  }


  if (mon > 10) {
    var stake_mon = mon - getRandomDelay(10000, 20000)/10000
    up = Array.from(document.querySelectorAll("input"));
    if (up.length != 0){
        const lastValue = up[0].value;
        up[0].value = stake_mon;
