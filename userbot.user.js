...
// @updateURL    https://github.com/IvanAgafonov/test-violentmonkey/raw/main/userbot.user.js
...
async function autoBuy() {
  ...
  var up = Array.from(document.querySelectorAll("span")).filter(el => el.textContent == "Camps");
  if (up.length != 0){
    triggerEvents(up[0]);
    // up[0].click();
  ...
  up = Array.from(document.querySelectorAll("span")).filter(el => el.textContent == "Home");
  if (up.length != 0){
    triggerEvents(up[0]);
    // up[0].click();
    await sleep(getRandomDelay(2000, 3000));
  ...
  up = Array.from(document.querySelectorAll("h6")).filter(el => el.textContent == "Start");
  if (up.length != 0){
  ...
  up = Array.from(document.querySelectorAll("h6")).filter(el => el.textContent == "Claim");
  if (up.length != 0){
    for (const item of up) {
      item.click();
      await sleep(getRandomDelay(2000, 5000));
...
function initializeScript() {
    ...
    console.log('START userbot claim')
