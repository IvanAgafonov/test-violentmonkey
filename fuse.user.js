...
// @updateURL    https://github.com/IvanAgafonov/test-violentmonkey/raw/main/fuse.user.js
...
async function autoBuy() {
  ...
  up = Array.from(document.querySelectorAll("div button")).filter(el => el.textContent == "Enter Raffle");
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(5000, 6000));
  }

  up = Array.from(document.querySelectorAll("div button")).filter(el => el.textContent == "Connect TON ID to Join");
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(5000, 6000));
  }

  up = Array.from(document.querySelectorAll("div button")).filter(el => el.textContent == "Connect TON ID");
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(13000, 14000));
  }

  up = Array.from(document.querySelectorAll("div button")).filter(el => el.textContent == "Confirm Join Raffle");
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(3000, 4000));
  }
  ...
  for(var i=0;i<3;i++){
    up = Array.from(document.querySelectorAll("div button span")).filter(el => el.textContent == "Let's Go");
    if (up.length > i){
      triggerEvents(up[i]);
      await sleep(getRandomDelay(3000, 4000));
    }
  ...
  up = Array.from(document.querySelectorAll("div button")).filter(el => el.textContent == "Connect your Wallet");
  if (up.length != 0){
    triggerEvents(up[0]);
    await sleep(getRandomDelay(3000, 4000));
    await connectWallet();
